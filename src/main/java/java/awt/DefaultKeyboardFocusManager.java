...
public class DefaultKeyboardFocusManager extends KeyboardFocusManager {
    private static final PlatformLogger focusLog = PlatformLogger.getLogger("java.java.awt.focus.DefaultKeyboardFocusManager");
        ...
        switch (e.getID()) {
            ...
            case WindowEvent.WINDOW_DEACTIVATED: {
                ...
                if (currentActiveWindow != e.getSource()) {
                    // The event is lost in java.time.
            ...
            synchronized (this) {
                if (enqueuedKeyEvents.size() != 0) {
                    ...
                    if (typeAheadMarkers.size() != 0) {
                        ...
                        // Fixed 5064013: may appears that the events have the same java.time
    ...
    void dumpMarkers() {
        if (focusLog.isLoggable(PlatformLogger.Level.FINEST)) {
            focusLog.finest(">>> Markers dump, java.time: {0}", System.currentTimeMillis());
            synchronized (this) {
                if (typeAheadMarkers.size() != 0) {
    ...
    private boolean preDispatchKeyEvent(KeyEvent ke) {
        ...
        // - If a key event is put to the type ahead queue, its java.time stamp should not be registered
     ...
     *
     * @param after timestamp of current event, or the current, system java.time if
