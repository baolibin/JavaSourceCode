...
class WaitDispatchSupport implements SecondaryLoop {
    ...
    private final static PlatformLogger log =
        PlatformLogger.getLogger("java.java.awt.event.WaitDispatchSupport");
     ...
     * @param interval A java.time interval to wait for. Note that
     *        when the waiting process takes place on EDT
     *        there is no guarantee to stop it in the given java.time
     *
     * @since 1.7
     */
    public WaitDispatchSupport(EventDispatchThread dispatchThread,
    ...
    public boolean enter() {
        ...
        if (currentThread == dispatchThread) {
            ...
            synchronized (getTreeLock()) {
                ...
                // java.time interval passed or an exception occurred, the state
                // should be changed
