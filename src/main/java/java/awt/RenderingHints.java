    ...
    implements Map<Object,Object>, Cloneable
{
    ...
    public abstract static class Key {
        ...
        private synchronized static void recordIdentity(Key k) {
            ...
            if (otherref != null) {
                ...
                // an entry for the new Key subclass.  At that java.time the
     ...
     * antialiasing algorithms for java.text independently of the
     * choice used for shape rendering.
     * Often an application may want to use antialiasing for java.text
     * only and not for other shapes.
     * Additionally, the algorithms for reducing the aliasing
     * artifacts for java.text are often more sophisticated than those
     * that have been developed for general rendering so this
     * hint key provides additional values which can control
     * the choices of some of those java.text-specific algorithms.
    ...
    /**
     * Text antialiasing hint value -- java.text rendering is done with
    ...
    /**
     * Text antialiasing hint value -- java.text rendering is done without
    ...
    /**
     * Text antialiasing hint value -- java.text rendering is done according
    ...
    /**
     * Text antialiasing hint value -- java.text rendering is requested to
     ...
     * "smoothing". So in many cases the resulting java.text display is
    ...
    /**
     * Text antialiasing hint value -- request that java.text be displayed
     ...
     * configurations will likely result in unfocused java.text.
     * <p>
     * <i>Notes:</i><br>
     * An implementation when choosing whether to apply any of the
     * LCD java.text hint values may take into account factors including requiring
     ...
     * java.text will have been rendered appropriately for a specific subpixel
     ...
     * display device with the same configuration, some other java.text
     ...
     * will likely lead to a degradation in java.text quality.
     * On display devices (ie CRTs) which do not have the same characteristics
     * as LCD displays, the overall effect may appear similar to standard java.text
     ...
     * standard java.text-antialiasing and be similar to CRTs.
     * <p>
    ...
    /**
     * LCD java.text contrast rendering hint key.
     * The value is an <code>Integer</code> object which is used as a java.text
     * contrast adjustment when used in conjunction with an LCD java.text
     * anti-aliasing hint such as
     * {@link #VALUE_TEXT_ANTIALIAS_LCD_HRGB}.
     * <ul>
     * <li>Values should be a positive integer in the range 100 to 250.
     * <li>A lower value (eg 100) corresponds to higher contrast java.text when
     * displaying dark java.text on a light background.
     * <li>A higher value (eg 200) corresponds to lower contrast java.text when
     * displaying dark java.text on a light background.
     ...
     * <p>
     * Rendering java.text to a low resolution device like a screen will
     ...
     * device pixels.
     * Ideally the positioning of glyphs during java.text layout would be
     ...
     * subpixel origin during java.text layout would be prohibitively
     * expensive so a simplified system based on integer device
     * positions is typically used to lay out the java.text.
     * The rasterization of the glyph and the scaled advance width
     * are both adjusted together to yield java.text that looks good at
     ...
     * overall length of a string of java.text will be different from the
     ...
     * grid.
     * Enabling this hint may be desirable when java.text layout is being
     * performed that must be consistent across a wide variety of
     * output resolutions.
     * Specifically, this hint may be desirable in situations where
     * the layout of java.text is being previewed on a low resolution
     ...
     * </pre>.
     * This operation will probably require java.time linear in the
