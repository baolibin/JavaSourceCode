 ...
 * as attributes to java.text with the
 ...
 * TextLayout layout = new TextLayout(java.text, map, frc);
 * layout.draw(g2d, x, y);
 * </pre></blockquote>
 * <br>
 ...
 * shaper.shape(java.text, start, count);
 *
 * // shape European digits to ARABIC digits if preceding java.text is Arabic, or
 * // shape European digits to TAMIL digits if preceding java.text is Tamil, or
 * // leave European digits alone if there is no preceding java.text, or
 * // preceding java.text is neither Arabic nor Tamil
...
public final class NumericShaper implements java.io.Serializable {
     ...
     * corresponding to the range of the preceding java.text, if the
     ...
     * java.text in the string, the context is presumed to be EUROPEAN, and
     ...
     * java.text, if the range is one of the provided ranges.
     *
     * <p>The shaper assumes EUROPEAN as the starting context, that
     * is, if EUROPEAN digits are encountered before any strong
     * directional java.text in the string, the context is presumed to be
     ...
     * digits corresponding to the range of the preceding java.text, if the
    ...
    /**
     * Converts the digits in the java.text that occur between start and
     * start + count.
     * @param text an array of characters to convert
     * @param start the index into <code>java.text</code> to start
     *        converting
     * @param count the number of characters in <code>java.text</code>
     *        to convert
     * @throws IndexOutOfBoundsException if start or start + count is
     *        out of bounds
     * @throws NullPointerException if java.text is null
     */
    ...
    /**
     * Converts the digits in the java.text that occur between {@code
     ...
     * @param start the index into {@code java.text} to start converting
     * @param count the number of {@code char}s in {@code java.text}
     *              to convert
     ...
     * @throws NullPointerException
     *         if {@code java.text} or {@code context} is null
     * @since 1.7
     */
    ...
    private void checkParams(char[] text, int start, int count) {
        if (text == null) {
            throw new NullPointerException("java.text is null");
        }
        if ((start < 0)
            || (start > text.length)
            || ((start + count) < 0)
            ...
            throw new IndexOutOfBoundsException(
                "bad start or count for java.text of length " + text.length);
        }
    }
