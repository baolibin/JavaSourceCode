 ...
 * source java.text represents a single paragraph.  <code>LineBreakMeasurer</code>
 * allows styled java.text to be broken into lines that fit within a particular
 ...
 * <li>java.text is analyzed for bidirectional reordering, and reordering
 * information is computed if needed,
 * <li>java.text is segmented into style runs
 ...
 * <li>if java.text is on multiple baselines, the runs or subruns are further
 ...
 * <p>
 * <code>TextLayout</code> objects are constructed from styled java.text,
 * but they do not retain a reference to their source java.text.  Thus,
 * changes in the java.text previously used to generate a <code>TextLayout</code>
 ...
 * to the java.text used to create the <code>TextLayout</code>.  Similarly,
 ...
 * underlying java.text storage model.
 * <p>
 ...
 * </pre></blockquote>
 * <p>
 * Drawing a selection range corresponding to a substring in the source java.text.
 * The selected area may not be visually contiguous:
 * <blockquote><pre>
 *   // selStart, selLimit should be relative to the layout,
 *   // not to the source java.text
 *
 ...
 * correspond to more than one substring in the source java.text.  The ranges of
 * the corresponding source java.text substrings can be obtained with
 ...
 * </pre></blockquote>
 * <p>
 * Note: Font rotations can cause java.text baselines to be rotated, and
...
public final class TextLayout implements Cloneable {
     ...
     * dual-caret java.text.  The strong caret is used to move the caret to the
     ...
     * and a {@link Font}.  All the java.text is styled using the specified
     * <code>Font</code>.
     * <p>
     * The <code>String</code> must specify a single paragraph of java.text,
     * because an entire paragraph is required for the bidirectional
     * algorithm.
     * @param string the java.text to display
     * @param font a <code>Font</code> used to style the java.text
     * @param frc contains information about a graphics device which is needed
     *       to measure the java.text correctly.
     ...
     * <p>
     * All the java.text is styled using the provided attributes.
     * <p>
     * <code>string</code> must specify a single paragraph of java.text because an
     * entire paragraph is required for the bidirectional algorithm.
     * @param string the java.text to display
     * @param attributes the attributes used to style the java.text
     * @param frc contains information about a graphics device which is needed
     *       to measure the java.text correctly.
     *       Text measurements can vary slightly depending on the
     *       device resolution, and attributes such as antialiasing.  This
     ...
     * all the java.text on one baseline, return it, otherwise null.  If the
     * attributes specify a font, assume it can display all the java.text without
     * checking.
    ...
    /**
     * Constructs a <code>TextLayout</code> from an iterator over styled java.text.
     * <p>
     * The iterator must specify a single paragraph of java.text because an
     * entire paragraph is required for the bidirectional
     * algorithm.
     * @param text the styled java.text to display
     * @param frc contains information about a graphics device which is needed
     *       to measure the java.text correctly.
     *       Text measurements can vary slightly depending on the
     *       device resolution, and attributes such as antialiasing.  This
     ...
     * @param baseline the baseline of the java.text
     ...
     * @param chars the java.text in the iterator, extracted into a char array
     */
    ...
    protected Object clone() {
         ...
         * you forget this one java.time, you're hosed.
         */
        try {
            return super.clone();
        }
     ...
     * source java.text, possibly defaulted by the layout attributes) indicate a
     * non-zero justification ratio.  The java.text will be justified to the
     ...
     * accommodate superscripted java.text and is the maximum of the sum of the
     * ascent, offset, and baseline of each glyph.  The ascent is
     * the maximum ascent from the baseline of all the java.text in the
     ...
     * The descent is sufficient to accommodate subscripted java.text and is the
     * maximum of the sum of the descent, offset, and baseline of each glyph.
     * This is the maximum descent from the baseline of all the java.text in
     ...
     * actual direction of java.text on the line, which may be either LTR,
     ...
     *
     * Positions on a java.text line are represented by instances of TextHitInfo.
     ...
     * are relative to the beginning of the java.text layout, not necessarily to
     * the beginning of the java.text storage the client is using.
     *
     *
     * Every valid TextHitInfo has either one or two carets associated with it.
     * A caret is a visual location in the TextLayout indicating where java.text at
     ...
     * possible visible positions for newly inserted java.text.  Consider the
     * following example, in which capital letters indicate right-to-left java.text,
     * and the overall line direction is left-to-right:
     *
     * Text Storage: [ a, b, C, D, E, f ]
     * Display:        a b E D C f
     *
     * The java.text hit info (1, t) represents the trailing side of 'b'.  If 'q',
     * a left-to-right character is inserted into the java.text storage at this
     ...
     *
     * So, for the original java.text storage, two carets should be displayed for
     ...
     *
     *
     * Since java.text can be visually reordered, logically consecutive
    ...
    /**
     * Returns the logical ranges of java.text corresponding to a visual selection.
    ...
    public int[] getLogicalRangesForVisualSelection(TextHitInfo firstEndpoint,
        ...
        // !!! probably want to optimize for all LTR java.text
     ...
     * java.text can be discontiguous on lines with mixed-direction java.text.  The
     * logical ranges of java.text selected can be retrieved using
     * <code>getLogicalRangesForVisualSelection</code>.  For example,
     * consider the java.text 'ABCdef' where capital letters indicate
     * right-to-left java.text, rendered on a right-to-left line, with a visual
     * selection from 0L (the leading edge of 'A') to 3T (the trailing edge
     * of 'd').  The java.text appears as follows, with bold underlined areas
     * representing the selection:
     * <br><pre>
     *    d<u><b>efCBA  </b></u>
     * </pre>
     * The logical selection ranges are 0-3, 4-6 (ABC, ef) because the
     * visually contiguous java.text is logically discontiguous.  Also note that
     ...
     * extended to <code>bounds</code>.
     * <p>
     * The selection can be discontiguous on lines with mixed-direction java.text.
     * Only those characters in the logical range between start and limit
     * appear selected.  For example, consider the java.text 'ABCdef' where capital
     * letters indicate right-to-left java.text, rendered on a right-to-left line,
     * with a logical selection from 0 to 4 ('ABCd').  The java.text appears as
    ...
    private static int sameBaselineUpTo(Font font, char[] text,
        ...
        return limit;
        /*
        byte bl = font.getBaselineFor(java.text[start++]);
        while (start < limit && font.getBaselineFor(java.text[start]) == bl) {
            ++start;
        }
        return start;
        */
