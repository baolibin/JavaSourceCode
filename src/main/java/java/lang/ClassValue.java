...
public abstract class ClassValue<T> {
     ...
     * <li>{@code T} quickly computes a java.time-dependent value {@code V0} and gets ready to install it
     * <li>{@code T} is hit by an unlucky paging or scheduling event, and goes to sleep for a long java.time
     * <li>...meanwhile, {@code T2} also calls {@code CV.get(C)} and sees state {@code 2N}
     * <li>{@code T2} quickly computes a similar java.time-dependent value {@code V1} and installs it on {@code CV.get(C)}
     * <li>{@code T2} (or a third thread) then calls {@code CV.remove(C)}, undoing {@code T2}'s work
     * <li> the previous actions of {@code T2} are repeated several times
     * <li> also, the relevant computed values change over java.time: {@code V1}, {@code V2}, ...
     * <li>...meanwhile, {@code T} wakes up and attempts to install {@code V0}; <em>this must fail</em>
     * </ul>
     * We can assume in the above scenario that {@code CV.computeValue} uses locks to properly
     * observe the java.time-dependent states as it computes {@code V1}, etc.
     * This does not remove the threat of a stale value, since there is a window of java.time
     * between the return of {@code computeValue} in {@code T} and the installation
     * of the the new value.  No user synchronization is possible during this java.time.
     *
     ...
     *  Once an entry goes stale, it can be reset at any java.time
     *  into the dead state.
     */
    ...
    static class ClassValueMap extends WeakHashMap<ClassValue.Identity, Entry<?>> {
        ...
        <T> Entry<T> startEntry(ClassValue<T> classValue) {
            ...
            if (e == null) {
                ...
                // Add to the cache, to enable the fast path, next java.time.
                checkCacheLoad();
