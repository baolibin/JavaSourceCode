 ...
 *    <tt>java.java.lang:type=MemoryPool</tt>}<tt>,name=</tt><i>pool's name</i>
 * </blockquote>
 *
 ...
 * memory pool basis at its best appropriate java.time, typically,
 * at garbage collection java.time.
 * Each memory pool maintains a {@link #getUsageThresholdCount
 * usage threshold count} that will get incremented
 * every java.time when the Java virtual machine
 ...
 * and mostly are unreachable at garbage collection java.time.
 ...
 *       changes during the action processing java.time.
 *
 *       <pre>
 *       // Assume the usage threshold is supported for this pool.
 ...
 *       at the point in java.time when the notification was constructed.
 ...
 * java.time, some number of bytes in the memory pools that are garbaged
 ...
 * checking at garbage collection java.time.
 *
 * <p>
 ...
 * garbage collection java.time which is the best appropriate java.time
...
public interface MemoryPoolMXBean extends PlatformManagedObject {
     ...
     * will check the memory usage at its best appropriate java.time after it has
