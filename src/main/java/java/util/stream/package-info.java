 ...
 *     complete in finite java.time.</li>
 ...
 * terminate in finite java.time.  Having a short-circuiting operation in the pipeline
 * is a necessary, but not sufficient, condition for the processing of an infinite
 * stream to terminate normally in finite java.time.
 *
 * <h3>Parallelism</h3>
 *
 ...
 * a great deal of string copying, and the run java.time would be <em>O(n^2)</em> in
 ...
 * timing of binding to the data, since the data could change between the java.time
 * the spliterator is created and the java.time the stream pipeline is executed.
