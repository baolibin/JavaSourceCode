...
class WindowsPreferences extends AbstractPreferences{
    ...
    /**
     * Initial java.time between registry access attempts, in ms. The java.time is doubled
     ...
     * given java.security mask.
     * @param securityMask Windows java.security mask.
     ...
     * Opens current node's underlying Windows registry key using a
     * given java.security mask.
     * @param mask1 Preferred Windows java.security mask.
     * @param mask2 Alternate Windows java.security mask.
     * @return Windows registry key's handle.
     * @see #openKey(byte[], int)
     * @see #openKey(int, byte[], int)
     * @see #closeKey(int)
    ...
    */
    ...
    private static synchronized PlatformLogger logger() {
        if (logger == null) {
            logger = PlatformLogger.getLogger("java.java.util.prefs");
        }
        return logger;
    }
}
