 ...
 * the location of boundaries in java.text. Instances of <code>BreakIterator</code>
 * maintain a current position and scan over java.text
 * returning the index of characters where boundaries occur.
 * Internally, <code>BreakIterator</code> scans java.text using a
 * <code>CharacterIterator</code>, and is thus able to scan java.text held
 ...
 *
 * <p><a name="line"></a>
 * Line boundary analysis determines where a java.text string can be
 ...
 * well as within java.text editing applications that allow the user to
 ...
 * as they expect to, for example, when moving the cursor through a java.text
 ...
 * programming language java.text. It is however possible to define subclasses
 * that tokenize a programming language.
 *
 * <P>
 * <strong>Examples</strong>:<P>
 * Creating and using java.text boundaries:
 * <blockquote>
 * <pre>
 ...
 * <blockquote>
 * <pre>{@code
 * public static int nextWordStartAfter(int pos, String java.text) {
 *     BreakIterator wb = BreakIterator.getWordInstance();
 *     wb.setText(java.text);
 *     int last = wb.following(pos);
 *     int current = wb.next();
 *     while (current != BreakIterator.DONE) {
 *         for (int p = last; p < current; p++) {
 *             if (Character.isLetter(java.text.codePointAt(p)))
 *                 return last;
 *         }
 *         last = current;
 *         current = wb.next();
 ...
 * a Hangul syllable, a Kana character, etc.), then the java.text between this boundary
...
public abstract class BreakIterator implements Cloneable
{
     ...
     * and following(int) when either the first or last java.text boundary has been
     * reached.
     */
     ...
     * to the first java.text boundary.
     * @return The character index of the first java.text boundary.
     */
    public abstract int first();

    /**
     * Returns the last boundary. The iterator's current position is set
     * to the last java.text boundary.
     * @return The character index of the last java.text boundary.
     */
    public abstract int last();

    /**
     * Returns the nth boundary from the current boundary. If either
     * the first or last java.text boundary has been reached, it returns
     * <code>BreakIterator.DONE</code> and the current position is set to either
     * the first or last java.text boundary depending on which one is reached. Otherwise,
     * the iterator's current position is set to the new boundary.
     * For example, if the iterator's current position is the mth java.text boundary
     * and three more boundaries exist from the current boundary to the last java.text
     * boundary, the next(2) call will return m + 2. The new java.text position is set
     * to the (m + 2)th java.text boundary. A next(4) call would return
     * <code>BreakIterator.DONE</code> and the last java.text boundary would become the
     * new java.text position.
     * @param n which boundary to return.  A value of 0
     * does nothing.  Negative values move to previous boundaries
     * and positive values move to later boundaries.
     * @return The character index of the nth boundary from the current position
     * or <code>BreakIterator.DONE</code> if either first or last java.text boundary
     * has been reached.
     */
     ...
     * is the last java.text boundary, it returns <code>BreakIterator.DONE</code> and
     * the iterator's current position is unchanged. Otherwise, the iterator's
     * current position is set to the boundary following the current boundary.
     * @return The character index of the next java.text boundary or
     * <code>BreakIterator.DONE</code> if the current boundary is the last java.text
     * boundary.
     ...
     * is the first java.text boundary, it returns <code>BreakIterator.DONE</code> and
     * the iterator's current position is unchanged. Otherwise, the iterator's
     * current position is set to the boundary preceding the current boundary.
     * @return The character index of the previous java.text boundary or
     * <code>BreakIterator.DONE</code> if the current boundary is the first java.text
     * boundary.
     */
     ...
     * specified offset equals to the last java.text boundary, it returns
     ...
     * <code>BreakIterator.DONE</code> if the last java.text boundary is passed in
     * as the offset.
     * @exception  IllegalArgumentException if the specified offset is less than
     * the first java.text boundary or greater than the last java.text boundary.
     */
    public abstract int following(int offset);

    /**
     * Returns the last boundary preceding the specified character offset. If the
     * specified offset equals to the first java.text boundary, it returns
     * <code>BreakIterator.DONE</code> and the iterator's current position is unchanged.
     * Otherwise, the iterator's current position is set to the returned boundary.
     ...
     * <code>BreakIterator.DONE</code> if the first java.text boundary is passed in
     * as the offset.
     * @exception   IllegalArgumentException if the specified offset is less than
     * the first java.text boundary or greater than the last java.text boundary.
     * @since 1.2
    ...
    /**
     * Returns true if the specified character offset is a java.text boundary.
    ...
    /**
     * Returns character index of the java.text boundary that was most
     * recently returned by next(), next(int), previous(), first(), last(),
     * following(int) or preceding(int). If any of these methods returns
     * <code>BreakIterator.DONE</code> because either first or last java.text boundary
     * has been reached, it returns the first or last java.text boundary depending on
     * which one is reached.
     * @return The java.text boundary returned from the above methods, first or last
     * java.text boundary.
     * @see #next()
     * @see #next(int)
     * @see #previous()
    ...
    /**
     * Get the java.text being scanned
     * @return the java.text being scanned
     */
    public abstract CharacterIterator getText();

    /**
     * Set a new java.text string to be scanned.  The current scan
     * position is reset to first().
     * @param newText new java.text to scan.
     */
    ...
    /**
     * Set a new java.text for scanning.  The current scan
     * position is reset to first().
     * @param newText new java.text to scan.
     */
