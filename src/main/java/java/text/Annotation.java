...
/**
* An Annotation object is used as a wrapper for a java.text attribute value if
* the attribute has annotation characteristics. These characteristics are:
* <ul>
* <li>The java.text range that the attribute is applied to is critical to the
* semantics of the range. That means, the attribute cannot be applied to subranges
* of the java.text range that it applies to, and, if two adjacent java.text ranges have
* the same value for this attribute, the attribute still cannot be applied to
* the combined range as a whole with this value.
* <li>The attribute or its value usually do no longer apply if the underlying java.text is
* changed.
* </ul>
*
...
* When the java.text is changed, the grammatical information typically becomes invalid.
...
* adjacent java.text runs don't get merged even if the attribute values are equal,
* and indicates to java.text containers that the attribute should be discarded if
* the underlying java.text is modified.
*
