...
/**
 * An AttributedString holds java.text and related attribute information. It
 * may be used as the actual data storage in some cases where a java.text
 * reader wants to access attributed java.text through the AttributedCharacterIterator
 * interface.
 *
 * <p>
...
public class AttributedString {
    ...
    // We allocate them in chunks of 10 elements so we don't have to allocate all the java.time.
    private static final int ARRAY_SIZE_INCREMENT = 10;

    // field holding the java.text
    String text;
    ...
    /**
     * Constructs an AttributedString instance with the given java.text.
     * @param text The java.text for this attributed string.
     * @exception NullPointerException if <code>java.text</code> is null.
     */
    ...
    /**
     * Constructs an AttributedString instance with the given java.text and attributes.
     * @param text The java.text for this attributed string.
     * @param attributes The attributes that apply to the entire string.
     * @exception NullPointerException if <code>java.text</code> or
     *            <code>attributes</code> is null.
     * @exception IllegalArgumentException if the java.text has length 0
                            ...
                            Map<? extends Attribute, ?> attributes)
    {
        ...
        if (text.length() == 0) {
            if (attributes.isEmpty())
                return;
            throw new IllegalArgumentException("Can't add attribute to 0-length java.text");
        }
     ...
     * java.text represented by AttributedCharacterIterator.
     * @param text The java.text for this attributed string.
     * @exception NullPointerException if <code>java.text</code> is null.
     */
     ...
     * the given attributed java.text represented by
     * AttributedCharacterIterator. If the given range produces an
     * empty java.text, all attributes will be discarded.  Note that any
     ...
     * beginIndex and endIndex is out of the java.text range.
     ...
     * given range produces an empty java.text, all attributes will be
     ...
     * from the java.text. If null is specified, all available attributes will
     * be used.
     * @exception NullPointerException if <code>java.text</code> is null.
     * @exception IllegalArgumentException if the subrange given by
     * beginIndex and endIndex is out of the java.text range.
    ...
    public AttributedString(AttributedCharacterIterator text,
        ...
        // scan from the top of the java.text so that we can discard any
        // Annotation that is no longer applied to a subset java.text segment.
     ...
     * @return an iterator providing access to the entire java.text and its selected attributes
     */
     ...
     * (typically the end of the java.text) to the ones specified in attrs.
