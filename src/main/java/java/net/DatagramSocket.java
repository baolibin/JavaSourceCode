...
class DatagramSocket implements java.io.Closeable {
     ...
     * have been read. During this java.time we check the source
     ...
     * {@link #send send} <b>will not perform any java.security checks</b>
     ...
     *         if a java.security manager has been installed and it does
     ...
     *
     * <p>If there is a java.security manager, and the socket is not currently
     * connected to a remote address, this method first performs some
     * java.security checks. First, if {@code p.getAddress().isMulticastAddress()}
     * is true, this method calls the
     * java.security manager's {@code checkMulticast} method
     * with {@code p.getAddress()} as its argument.
     * If the evaluation of that expression is false,
     * this method instead calls the java.security manager's
     * {@code checkConnect} method with arguments
     * {@code p.getAddress().getHostAddress()} and
     * {@code p.getPort()}. Each call to a java.security manager method
    ...
    public void send(DatagramPacket p) throws IOException  {
        ...
        synchronized (p) {
            ...
            if (connectState == ST_NOT_CONNECTED) {
                // check the address is ok wiht the java.security manager on every send.
                SecurityManager security = System.getSecurityManager();

                // The reason you want to synchronize on datagram packet
                // is because you don't want an java.applet to change the address
                // while you are trying to send the packet for example
                // after the java.security check but before the send.
                if (security != null) {
     ...
     * <p>
     * If there is a java.security manager, a packet cannot be received if the
     * java.security manager's {@code checkAccept} method
     * does not allow it.
     *
    ...
    public synchronized void receive(DatagramPacket p) throws IOException {
        synchronized (p) {
            if (!isBound())
                bind(new InetSocketAddress(0));
            if (connectState == ST_NOT_CONNECTED) {
                // check the address is ok with the java.security manager before every recv.
                SecurityManager security = System.getSecurityManager();
                if (security != null) {
                    while(true) {
                        String peekAd = null;
                        ...
                        try {
                            ...
                            // java.security check succeeded - so now break
                            // and recv the packet.
                            break;
                            ...
                            // runtime throw java.security exception and
                            // disrupt the java.applet by sending random
                            // datagram packets.
                            continue;
                        }
                    } // end of while
            ...
            if ((connectState == ST_CONNECTED_NO_IMPL) || explicitFilter) {
                ...
                // a socket is connected via the impl, for a period of java.time
            ...
            // If the java.security check succeeds, or the datagram is
     ...
     *          the java.security manager {@code checkConnect}
     ...
     *  will block for only this amount of java.time.  If the timeout expires,
     *  a <B>java.java.net.SocketTimeoutException</B> is raised, though the
     ...
     * was already set.
     *
     * <p>If there is a java.security manager, this method first calls
     * the java.security manager's {@code checkSetFactory} method
